# Projeto Operação Carcará - Painel de Desempenho 🚀

## Visão Geral

Esta aplicação web foi desenvolvida como uma ferramenta de **accountability e gamificação** para o grupo de estudos "Operação Carcará", focado na preparação para concursos públicos (principalmente PMBA). O objetivo principal é centralizar o registro de desempenho dos alunos, promovendo a **constância, a análise individual e a competição saudável** dentro do grupo.
...
Atualmente, a aplicação está **ativa e em uso** pelo grupo Operação Carcará. Serve como uma ferramenta diária de acompanhamento e motivação.
